...
Rockstar is intended to give programmers an unprecedented degree of poetic licence when it comes to the composition and structure of their programs. 
...
There are two ways to declare and use variables in Rockstar. 
...
Rockstar also supports a unique language feature known as **poetic literals**. Inspired by the [here-document](https://en.wikipedia.org/wiki/Here_document) syntax supported by many scripting languages, poetic literals allow programmers to simultaneously initialize a variable and express their innermost angst.
